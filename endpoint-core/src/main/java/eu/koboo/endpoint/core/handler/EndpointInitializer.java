...
public class EndpointInitializer extends ChannelInitializer<Channel> {
  ...
  private final EventExecutorGroup processExecutor = new DefaultEventExecutorGroup(
      EndpointCore.CORES * 2);
  ...
  public void initChannel(Channel ch) {
    try {
      ...
      if (endpoint.builder().isFraming()) {
        pipeline.addLast(LENGTH_DECODER,
      ...
      if (endpoint.builder().getCompression() != null
          && endpoint.builder().getCompression() != Compression.NONE) {
      ...
      if (endpoint.builder().isUsingTimeouts()) {
        pipeline.addLast(IDLE_STATE_THROWER,
            new IdleStateHandler(endpoint.builder().getReadTimeout(),
                endpoint.builder().getWriteTimeout(), 0));
      ...
      if (endpoint.builder().isProcessing() && (!processExecutor.isShutdown()
          && !processExecutor.isTerminated())) {
        pipeline.addLast(processExecutor, ENDPOINT_HANDLER,
            new EndpointHandler(endpoint, channels));
      } else {
