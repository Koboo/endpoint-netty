...
public class BufUtils {
  ...
  public static String readStringCap(ByteBuf buf, int cap) {
    int length = readVarInt(buf);
      ...
      if (length > cap * 4) {
          throw new IllegalArgumentException(
              "Bad string size (got " + length + ", maximum is " + cap + ")");
      }
      if (!buf.isReadable(length)) {
          throw new IllegalArgumentException(
              "Trying to read a string that is too long (wanted " + length + ", only have "
                  + buf.readableBytes() + ")");
      }
    ...
    buf.skipBytes(length);
      if (str.length() > cap) {
          throw new IllegalArgumentException(
              "Got a too-long string (got " + str.length() + ", max " + cap + ")");
      }
    return str;
  }
  ...
  public static void writeStringCap(ByteBuf buffer, String string) {
    ...
    if (values.length > 32767) {
      throw new IllegalArgumentException(
          "String too big (was " + string.length() + " bytes encoded, max " + 32767 + ")");
    } else {
  ...
  public static byte[] readArrayCap(ByteBuf buf, int cap) {
    int length = readVarInt(buf);
      ...
      if (length > cap) {
          throw new IllegalArgumentException(
              "Bad array size (got " + length + ", maximum is " + cap + ")");
      }
      if (!buf.isReadable(length)) {
          throw new IllegalArgumentException(
              "Trying to read an array that is too long (wanted " + length + ", only have "
                  + buf.readableBytes() + ")");
      }
  ...
  public static byte[] readArray(ByteBuf input) {
    ...
    if (len > input.readableBytes()) {
      throw new IndexOutOfBoundsException(
          String.format("Cannot receive byte array longer than %s (got %s bytes)",
              input.readableBytes(), len));
    }
